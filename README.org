...
The following currently needs to be run manually, until I figure out how to
atomate this process. There is an issue with requiring the screen when positioning
the glyphs.
...
Then in magic
#+begin_export ascii
  path .:libraries/sky130_pschulz_xx_hd/mag:magic
  source magic/skywater.tcl
